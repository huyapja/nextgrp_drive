...
def add_user_to_drive_team(doc, method):
    ...
    try:
        ...
        if not organization_name:
            frappe.log_error(
                message="Không tìm thấy tổ chức nào trong hệ thống", title="No Organization"
            )
            return
        ...
        frappe.logger().info(
            f"Đã thêm user {doc.name} vào Drive Team {team.name} và tạo Drive Settings thành công"
        )

    except Exception as e:
        frappe.log_error(
            message=frappe.get_traceback(), title=f"Drive Team Hook Error - User: {doc.name}"
        )
...
def _get_organization_name():
    ...
    if not organization_name:
        organizations = frappe.get_all(
            "Organization", limit=1, order_by="creation asc", fields=["name"], pluck="name"
        )
...
def _add_user_to_team(team, user_email):
    ...
    team.append("users", {"user": user_email, "access_level": 1})
...
def _create_drive_settings(user_email, default_team):
    ...
    try:
        ...
        frappe.logger().info(
            f"Đã tạo Drive Settings cho user {user_email} với default team {default_team}"
        )
